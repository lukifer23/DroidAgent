...
DroidAgent is a macOS-first, single-owner control plane for OpenClaw with a mobile-first PWA shell for chat, files, jobs, runtime control, passkeys, Tailscale phone access, optional Signal ingress, local multimodal attachments, and a unified owner decision inbox.
...
- `packages/shared`
  - shared Zod contracts for dashboard, auth, files, jobs, channels, owner decisions, access state, and diagnostics telemetry
...
- durable memory writes stay owner-reviewed through editable memory drafts before they append to `MEMORY.md`, `PREFERENCES.md`, or `memory/YYYY-MM-DD.md`
...
- Server-side mutation fanout now converges through one slice-aware realtime queue so dashboard invalidation, startup cache refresh, and websocket event bursts stay coalesced instead of fanning out through one-off publish paths.
...
- Owner-gated actions now flow through one decision model. OpenClaw exec approvals, memory-draft review, and Signal pairing all surface in the same inbox, while existing compatibility routes remain intact.
...
- Client chat state is now canonicalized in one per-session store for loaded history, optimistic sends, active run state, streaming text, recent feedback, and session-switch state instead of splitting that lifecycle across app context, websocket plumbing, and separate run/stream stores.
- The server now tracks OpenClaw on the patched `2026.3.31` release line and pins the nested `@anthropic-ai/sdk` plus `lodash@4.18.1` transitive dependencies to patched versions, so the production dependency audit stays clean without forking OpenClaw behavior in-repo.
...
- Route-switch perf timing now starts at in-browser route activation and measures until the target route control is visible, which removes Playwright actionability overhead from the deterministic route budget and keeps the metric tied to app work.
